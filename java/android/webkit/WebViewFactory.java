...
public final class WebViewFactory {
    ...
    private static final String WEBKIT_WEBVIEW_FACTORY =
    ...
    static WebViewFactoryProvider getProvider() {
        return getWebKitWebViewProvider();
    }

    private static WebViewFactoryProvider getChromiumWebViewProvider() {
    ...
    private static WebViewFactoryProvider getWebKitWebViewProvider() {
        synchronized (sProviderLock) {
            // For now the main purpose of this function (and the factory abstraction) is to keep
            // us honest and minimize usage of WebView internals when binding the proxy.
            if (sProviderInstance != null) return sProviderInstance;
            ...
            try {
                ...
                Trace.traceBegin(Trace.TRACE_TAG_WEBVIEW, "WebViewFactory.getFactoryClass()");
                try {
                    providerClass = (Class<WebViewFactoryProvider>) Class.forName(WEBKIT_WEBVIEW_FACTORY);;
                } catch (ClassNotFoundException e) {
                    Log.e(LOGTAG, "error loading provider", e);
                    throw new AndroidRuntimeException(e);
                } finally {
